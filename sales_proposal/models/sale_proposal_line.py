...
class SaleProposalLine(models.Model):
    ...
    _description = "Sale Propsal Line"    
    
    sale_proposal_id = fields.Many2one(
        comodel_name='sale.proposal',
        string="Order Reference",
        required=True, ondelete='cascade', index=True, copy=False)
    order_partner_id = fields.Many2one(
        related='sale_proposal_id.partner_id',
        string="Customer",
    ...
    company_id = fields.Many2one(related='sale_proposal_id.company_id',store=True)
    currency_id = fields.Many2one(
        related='sale_proposal_id.currency_id',
        depends=['sale_proposal_id.currency_id'],
    ...
    def _compute_price_subtotal_approved(self):
        if self.sale_proposal_id.state == 'draft':
            for line in self:
    ...
    def _compute_price_unit(self):
        for line in self:
            if not line.product_uom or not line.product_id or not line.sale_proposal_id.pricelist_id:
                line.price_unit = 0.0
            else:
                price = line.with_company(line.company_id)._get_display_price()
                line.price_unit = line.product_id._get_tax_included_unit_price(
                    line.company_id,
                    line.sale_proposal_id.currency_id,
                    line.sale_proposal_id.date_order,
                    'sale',
                    fiscal_position=line.sale_proposal_id.fiscal_position_id,
    ...
    def _get_pricelist_price(self):
        ...
        order_date = self.sale_proposal_id.date_order or fields.Date.today()
    ...
    def _convert_to_tax_base_line_dict(self):
        ...
        return self.env['account.tax']._convert_to_tax_base_line_dict(
            ...
            currency=self.sale_proposal_id.currency_id,
            product=self.product_id,
            taxes=self.tax_id,
    ...
    def _compute_amount_approved(self):
        for line in self:
            tax_results_approved = self.env['account.tax']._compute_taxes([line._convert_to_tax_base_line_dict_approved()])
