...
class SaleProposal(models.Model):
    ...
    sale_proposal_line = fields.One2many(
        comodel_name='sale.proposal.line',
        inverse_name='sale_proposal_id',
    ...
    amount_untaxed_approved = fields.Monetary(string="Approved Untaxed Amount", store=True, compute='_compute_amounts_approved', tracking=5)
    amount_tax_approved = fields.Monetary(string="Taxes", store=True, compute='_compute_amounts_approved')
    amount_total_approved = fields.Monetary(string="Total", store=True, compute='_compute_amounts_approved', tracking=4)
    ...
    def action_send_proposal_mail(self):
        ...
        mail_template = self.env.ref('sale_proposal.email_template_sale_proposal', raise_if_not_found=False)
    ...
    @api.depends('sale_proposal_line.price_subtotal', 'sale_proposal_line.price_tax', 'sale_proposal_line.price_total')
    def _compute_amounts(self):
        for rec in self:
            order_lines = rec.sale_proposal_line.filtered(lambda x: not x.display_type)
    ...
    @api.depends('sale_proposal_line.tax_id', 'sale_proposal_line.price_unit', 'amount_total', 'amount_untaxed')
    ...
    @api.depends('sale_proposal_line.price_subtotal_approved', 'sale_proposal_line.price_tax', 'sale_proposal_line.price_total_approved')
    ...
    @api.depends('sale_proposal_line.tax_id', 'sale_proposal_line.price_unit_approved', 'amount_total_approved', 'amount_untaxed_approved')
    ...
    def _portal_reback_action(self):
        self.ensure_one()
        return self.env.ref('sale_proposal.sale_proposal_action')
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                ...
                vals['name'] = seq_date.strftime("Proposal/%Y/%b/") + self.env['ir.sequence'].next_by_code(
                    'sale.proposal') if seq_date else 'New'
    ...
    def action_preview_sale_proposal(self):
        ...
        return {
            ...
            'target': 'self',
            'url': f'/my/proposals/{self.id}/{self.get_portal_url()}',
        }
        ...
        self.order_id = self.env['sale.order'].create(
         [{ 
            ...
            'order_line':  [Command.create({
                ...
                })for line in self.sale_proposal_line]
          }]    
        )
        self.state = 'confirm'
        return True