...
class ProposalPortal(portal.CustomerPortal):
    ...
    def _get_proposal_portal_details(self, proposal):
        return  {
            'proposal_template': request.env['ir.ui.view']._render_template(
                'sale_proposal.sale_proposal_portal_content', {
    ...
    def _sale_proposal_sortings(self):
        return {
    ...
    def _proposal_portal_rendering_data(
        ...
        searchbar_sortings = self._sale_proposal_sortings()
        ...
        orders = SaleProposal.search(domain, order=sort_order, limit=self._items_per_page, offset=pager_values['offset'])
        values.update({
            'date': date_begin,
            'quotations': orders.sudo(),
            'orders': orders.sudo(),
            'page_name': 'sale_proposal',
            'pager': pager_values,
            'default_url': url,
    ...
    def portal_my_proposal(self, **kwargs):
        ...
        return request.render("sale_proposal.portal_my_sale_proposals", values)
    
    @http.route(['/my/proposals/<int:order_id>'], type='http', auth="public", website=True)
    def portal_sale_proposal_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access('sale.proposal', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        
        if report_type in ('html', 'pdf', 'text'):
            return self._show_report(model=order_sudo, report_type=report_type, report_ref='sale_proposal.action_report_saleorder', download=download)
    ...
    @http.route(['/my/proposals/<int:order_id>/approve'], type='http', auth="public", methods=['POST'], website=True)
    ...
    @http.route(['/my/proposals/<int:order_id>/update_line_dict'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    def update_orderlines_json(self, order_id,access_token=None,line_id=None,input_quantity=None,input_price=None, **kw):
        try:
            proposal = self._document_check_access('sale.proposal', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        
        if proposal.state not in ('draft', 'send'):
            return False
        order_line = request.env['sale.proposal.line'].sudo().browse(int(line_id)).exists()
        
        if not order_line or order_line.sale_proposal_id != proposal:
            return False
